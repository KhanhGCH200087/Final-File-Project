...
const MCoordinatorDetail = () => {
    ...
    useEffect(() => {
        const fetchStudent = async () => {
            try {
                ...
                setValues({
                    ...values, email: res.data.user.email, name: res.data.marketingcoordinator.name, gender: res.data.marketingcoordinator.gender,
                    address: res.data.marketingcoordinator.address, faculty: res.data.marketingcoordinator.faculty._id, image: res.data.marketingcoordinator.image, dob: res.data.marketingcoordinator.dob
                })
            } catch (error) {
                console.log(error)
            }
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.put(`${apiUrl}/marketingcoordinator/edit/` + id, values)
            console.log(res)
    ...
    const handleDelete = async () => {
        try {
            if (window.confirm("Are you sure delete this?")) {
                const res = await axios.delete(`${apiUrl}/marketingcoordinator/delete/` + id)
                navigate('/mcoordinator')
            }
        } catch (error) {
            console.log(error)
        ...
        <div className="container add">
            ...
            <Form>
                ...
                <Form.Select
                    ...
                    name="faculty"
                >
                    <option hidden>{mcoordinator}</option>
                    {faculty.map((faculty) => {
                        return (
...
export default MCoordinatorDetail
